        ...
        const uint16_t NextPowerOfTwoAfterWidth)
{
    ...
    uint16_t biclusterHeight = memory.Sizes[GlobalID * 2 + 1];
    double cnrows = memory.gammaln[MatrixHeight + 1] - memory.gammaln[biclusterHeight + 1]
...
{
    sortSelectWidthAndColumns_kernel<<< activeInvocations, memory.NextPowerOfTwoAfterWidth/2,
