...
__global__ void sortSelectHeightAndRows_kernel(
        ...
        const uint16_t NextPowerOfTwoAfterHeight, const uint16_t TotalInvocations)
{
    ...
    uint32_t index = rowId1 * TotalInvocations + GlobalID;
    ...
    if (index < MatrixHeight)
    {
        ...
        index = rowId2 * TotalInvocations + GlobalID;
...
{
    sortSelectHeightAndRows_kernel<<< activeInvocations, memory.NextPowerOfTwoAfterHeight/2,
